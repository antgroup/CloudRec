...
is_internet_slb if {
	lower(sprintf("%v", [object.get(object.get(input, "LoadBalancer", {}), "AddressType", "")])) == "internet"
}

is_internet_slb if {
	lower(sprintf("%v", [object.get(object.get(input, "LoadBalancerAttribute", {}), "AddressType", "")])) == "internet"
}

is_internet_slb if {
	input.EipAddress != null
	count(input.EipAddress) > 0
}
